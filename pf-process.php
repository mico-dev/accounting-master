...
if($type=='prev') {
    ...
    $select = mysqli_query($con, "SELECT * FROM `tb_md_pf` WHERE pf_from_date='$from' AND pf_to_date='$to' AND d_license='$license'") or die(mysqli_error($con));
    ...
    $pcfrom = $_POST['pcfrom'];
    $pcto = $_POST['pcto'];
    $insert = mysqli_query($con, "INSERT INTO `tb_md_pf`(`patient_name`, `pf_amount`, `pf_trans_date`, `pf_from_date`, `pf_to_date`, `d_license`) VALUES ('$patientname', '$amount', '$transdate', '$pcfrom', '$pcto', '$license')") or die(mysqli_error($con));
    if($insert) {
        echo '1';
        die();
    } else {
    ...
    $id = $_POST['id'];
    $insert = mysqli_query($con, "DELETE FROM `tb_md_pf` WHERE id='$id'") or die(mysqli_error($con));
    ...
    <head>
      <meta charset='utf-8'>
        ...
        <title>MIHMCI PF Period Covered ".date('F Y', strtotime($from))."</title>
        <style>
    ...
    <caption style='border: 1px solid #06418e;background:#06418e;color:#000;font-size:36px;'>MIHMCI PF - <span style='color:#000;font-size:36px;'>Dr. ".$fullname."</span></caption>
    ...
    #$to_email      = $d_email;
    $subject = "MIHMCI PF Period Covered ".date('F Y', strtotime($from));
    $message = $msg;
    $headers = "From: admin\nMIME-Version: 1.0\nContent-Type: text/html; charset=utf-8\n";

    if(mail($to_email, $subject, $message, $headers)) {
        $update = mysqli_query($con, "UPDATE `tb_md_pf` SET `status`='sent' WHERE pf_from_date='$from' AND pf_to_date='$to' AND d_license='$d_license'") or die(mysqli_error($con));
    ...
    $msg="
        ...
        <head>
            ...
            <meta name='viewport' content='width=device-width'>
            <meta http-equiv='X-UA-Compatible' content='IE=edge'>
            <title>MIHMCI PF Mailer Archive - Professional Fee Period Covered ".date('F Y', strtotime($from))."</title>
        </head>  
        <body>
            ...
            <td style='border: 1px solid #000;color:#000;font-size:14px;text-align:left;'>MIHMCI PF Period Covered ".date('F Y', strtotime($from))."</td>
            ...
            $subject = "MIHMCI PF Mailer Archive - Professional Fee Period Covered ".date('F Y', strtotime($from));
            $message = $msg;
            $headers = "From: admin\nMIME-Version: 1.0\nContent-Type: text/html; charset=utf-8\n";
    //end archive        
            if(mail($to_email, $subject, $message, $headers)) { 
