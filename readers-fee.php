					...
					<div class="row">
						<div class="col-md-12">
							<div class="card card-dark bg-primary-gradient">
								...
								<h2 class="text-white pb-2">Readers Fee List</h2>
							<?php 
								...
								<form enctype="multipart/form-data" method="post" role="form" class="alert alert-success">
									<div class="form-group">
										<div class="custom-file">
											<label class="custom-file-label" style="color:#333 !important;">Choose "readersfee.csv" file...</label>
									...
									<div class="form-group">
										...
										if(isset($_POST["submit"])){
											...
											if($_FILES['file']['size'] > 0) {
												if($_FILES['file']['name'] == 'readersfee.csv') {
													$handle = fopen($file, "r");
													fgetcsv($handle); //skip 1st row
													$result = array();

														echo '<table class="table-bordered table-dark w-100">';
														$totalduplicate ='';
													while (($column = fgetcsv($handle, 100000, ",")) !== FALSE) {
														...
														#$encoder = $column[6];
														//duplicate-checker
															$duplicate = mysqli_query($con, "SELECT * FROM `tb_md_rf` WHERE patient_name='$patient_name' AND pf_amount='$pf_amount' AND pf_trans_date='$pf_trans_date' AND d_license='$d_license'")or die(mysqli_error($con));
															...
															if(mysqli_num_rows($duplicate)==0) {
																$filterlicense = mysqli_query($con, "SELECT * FROM tb_pf_dr WHERE d_license='$d_license'") or die(mysqli_error($con));
																	while($row=mysqli_fetch_assoc($filterlicense)) {
																		...
																		$sql = mysqli_query($con,"INSERT INTO `tb_md_rf`(`patient_name`, `pf_amount`, `pf_trans_date`, `pf_from_date`, `pf_to_date`, `d_license`) VALUES ('$patient_name', '$pf_amount', '$pf_trans_date', '$pf_from_date', '$pf_to_date','$filteredlicense')")or die(mysqli_error($con));
													...
													echo '<tr><td colspan="5">'.$totalduplicate.' TOTAL DUPLICATE FOUND</td></tr>';
													echo '</table>';
													fclose($handle);
												echo $result[0];
												} else {
										...
										if(!empty($searchdoctor)) {
											...
											if(mysqli_num_rows($querydoc)<>0) {
												while($row=mysqli_fetch_assoc($querydoc)) {
													...
													$filter = mysqli_query($con, "SELECT * FROM `tb_md_rf` WHERE pf_from_date='$from' AND pf_to_date='$to' AND d_license='$d_license' GROUP BY d_license ORDER BY `d_license` DESC") or die(mysqli_error($con));
												...
												$filter = mysqli_query($con, "SELECT * FROM `tb_md_rf` INNER JOIN tb_pf_dr ON tb_pf_dr.d_license =  tb_md_rf.d_license WHERE pf_from_date='$from' AND pf_to_date='$to' GROUP BY tb_md_rf.d_license ORDER BY tb_pf_dr.d_id") or die(mysqli_error($con));
	...
	$(document).ready(function() {
		...
		/*$(document).on('click', '.sendgroup', function(e) {
			...
			$('#modal').on('shown.bs.modal', function () {
				var type = 'sendgroup';
				var modal  = $(this);
				var title = '<p>Are you sure you want send Dr. '+drname+' his/her RF, period covered: '+from+' to '+to+' ?</p>';
				...
				$(document).on('click', '.yes', function(e) {
					...
					//modal.find('.modal-body').addClass('is-loading');
					var main = $.post('main-email.php', {from: from, to: to, license: license});
					var archive = $.post('archive-email.php', {from: from, to: to, license: license});
					
					$.when(main, archive).done(function (mainresult, archiveresult) {
	...
	<div class="modal-dialog" role="document" style="max-width:60%;">
		<div class="modal-content">
			<div class="modal-header bg-primary-gradient">
				<h3 class="modal-title text-light" id="title">MIHMCI RF Mailer</h3>
